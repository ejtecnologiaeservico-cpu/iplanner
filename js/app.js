// Funções Globais iRunner PLAN (Versão HTML/JS) - Estável para Múltiplos Usuários
...
function getStorageSafely(key, defaultValue) {
    try {
...
function getConfig() {
            return getStorageSafely('irunner_config', DEFAULT_CONFIG);
        }

function saveConfig(config) {
            return setStorageSafely('irunner_config', config);
        }

function getUsers() {
    return getStorageSafely('irunner_users', []);
}

function getWorkouts() {
    return getStorageSafely('irunner_workouts', []);
}

function getPayments() {
    return getStorageSafely('irunner_payments', []);
}

function saveUsers(users) {
    return setStorageSafely('irunner_users', users);
}

function saveWorkouts(workouts) {
    return setStorageSafely('irunner_workouts', workouts);
}

function savePayments(payments) {
    return setStorageSafely('irunner_payments', payments);
}

function getPerformanceMetrics() {
    return getStorageSafely('irunner_performance', []);
}

function savePerformanceMetrics(metrics) {
    return setStorageSafely('irunner_performance', metrics);
}
...
function checkInWorkout(workoutId) {
    try {
        ...
        if (workoutIndex !== -1) {
            workouts[workoutIndex].checkIn = new Date().toLocaleString('pt-BR', {
                day: '2-digit',
                month: '2-digit',
                year: 'numeric',
                hour: '2-digit',
...
function getUpcomingPayments(athleteId) {
    try {
...
function checkPaymentNotifications(athleteId) {
    try {
        const payments = getPayments();
        const athletePayments = payments.filter(p => p.athleteId === athleteId && p.status === 'pendente');
        const today = new Date();
...
function notifyTrainer(athleteName, paymentDetails) {
